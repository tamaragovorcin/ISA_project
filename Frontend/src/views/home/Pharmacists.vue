        ...
        <div style="background: white; height: 60px; margin-top: 20px">
          <span  style="float:right;margin:15px">
            <div class="input-group mb-3">
              <input type="text" v-model="pharmacistName" class="form-control" placeholder="Enter pharmacy name..." aria-label="Enter name..." aria-describedby="basic-addon2">
              <input type="text" v-model="phrmacistSurname" class="form-control" placeholder="Enter pharmacy surname..." aria-label="Enter name..." aria-describedby="basic-addon2">
...
export default {
  ...
  methods:{     
      ...
      markIsSelected : function(event, mark) { 
            ...
            this.axios.get('/pharmacist/searchMark/'+MarkMin+"/"+MarkMax).
                then(response => {
      ...
      pharmacyIsSelected : function(event, item) { 
          alert(item);
            this.axios.get('/pharmacist/searchPharmacy/'+item).
                then(response => {
                     this.pharmacists= response.data;
                }).catch(res => {
      ...
      showByPharmacistName : function() {
            this.axios.get('/pharmacist/searchName/'+this.pharmacistName+"/"+this.phrmacistSurname).
            then(response => {
                    this.pharmacists= response.data;
            }).catch(res => {
  ...
  mounted() {
       let token = localStorage.getItem('token').substring(1, localStorage.getItem('token').length-1);
        this.axios.get('/pharmacist/front',{ 
             headers: {
