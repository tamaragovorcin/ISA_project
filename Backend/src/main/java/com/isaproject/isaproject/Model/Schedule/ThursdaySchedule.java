...
public class ThursdaySchedule {
    ...
    @JsonManagedReference(value="schedule-thursday")
    @OneToMany(mappedBy = "thursdaySchedule", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    private Set<WorkingHoursPharmacist> workingHoursPharmacists = new HashSet<WorkingHoursPharmacist>();

    @JsonManagedReference(value="schedule-thursday1")
    @OneToMany(mappedBy = "thursdaySchedule", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    ...
    public ThursdaySchedule(LocalTime startTime, LocalTime endTime) {
