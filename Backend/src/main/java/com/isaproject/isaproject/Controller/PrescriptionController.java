...
public class PrescriptionController {
    ...
    public ResponseEntity<String> addPrescription(@RequestBody PrescriptionSendDTO sendDTO) {
        ...
        return prescription == null ?
                new ResponseEntity<>( "Medicine is not aveilable", HttpStatus.CREATED) :
    ...
    public ResponseEntity<String> addPrescription2(@RequestBody Prescription2DTO sendDTO) {
            ...
            Examination consulting = examinationService.getById(sendDTO.getExaminationId());
            PrescriptionDTO prescription = new PrescriptionDTO();

            prescription.setPatient(consulting.getPatient());
            prescription.setPharmacy(consulting.getExaminationSchedule().getPharmacy());
            prescription.setDate(consulting.getExaminationSchedule().getDate());
            prescription.setDurationOfTherapy(sendDTO.getDurationOfTherapy());
            prescription.setTaken(false);
            ...
            Boolean haveMedicine = false;
            MedicationPrice medicationPrice = new MedicationPrice();
            Set<MedicationPrice> medicationPrices = consulting.getExaminationSchedule().getPharmacy().getMedicationPrices();
            for (MedicationPrice mp: medicationPrices) {
                if (mp.getMedication().getId() == sendDTO.getMedication().getId() && mp.getQuantity() > 0) {
                    haveMedicine = true;
                    medicationPrice= mp;
            ...
            if(haveMedicine == false){
                LackMedicineDTO lackMedicineDTO = new LackMedicineDTO();
                Medication medication = medicationService.findById(sendDTO.getMedication().getId());
                System.out.println(medication.getName());
                lackMedicineDTO.setNameMedicine(medication.getName());
                lackMedicineDTO.setNamePharmacy(consulting.getExaminationSchedule().getPharmacy().getPharmacyName());
                lackMedicineService.save(lackMedicineDTO);
